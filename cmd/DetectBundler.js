...
const all = require("../Fingerprints/all.js");
...
async function detect(filecontent) {
    ...
    try {
        bigAst = parser.parse(currentFile.toString(), {
            ecmaVersion: 2020,
        }).body;
    } catch (e) {
    ...
    for (const FP of DetectionFPS) {
        ...
        for (let f of FP.detection) {
            let smallAst = parser.parse(f.fingerprint, {
                ecmaVersion: 2020,
            }).body;
...
(async () => {
    var args = yargs(process.argv.slice(2))
        ...
        .describe(
            "b",
            "list of bundlers to check for. Specified in fingerprints file"
        )
    ...
    // check for a custom fingerprint file
    if (args.fp) {
        // load file
        let;
    }
    ...
    for (const file of filepaths) {
        ...
        if (res[0]) {
            console.log(
                `File:${file} matches with ${res[2]} fingerprint which indicates bundler:${res[1]}`
            );
        }
